...
filepath_Si = os.path.join(module_dir,'QEdark-python/Si_f2.txt')
filepath_Ge = os.path.join(module_dir,'QEdark-python/Ge_f2.txt')

fcrys = {'Si': np.transpose(np.resize(np.loadtxt(filepath_Si,skiprows=1),(nE,nq))),
         'Ge': np.transpose(np.resize(np.loadtxt(filepath_Ge,skiprows=1),(nE,nq)))}
